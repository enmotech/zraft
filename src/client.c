...
int raft_barrier(struct raft *r, struct raft_barrier *req, raft_barrier_cb cb)
{
    ...
    if (r->state != RAFT_LEADER || r->transfer != NULL) {
        rv = RAFT_NOTLEADER;
        evtErrf("raft(%llx) apply barrier failed %d", r->id, rv);
        goto err;
    }
                  ...
                  raft_transfer_cb cb)
{
    ...
    if (r->state != RAFT_LEADER || r->transfer != NULL) {
        rv = RAFT_NOTLEADER;
        ErrMsgFromCode(r->errmsg, rv);
        evtErrf("raft(%llx) transfer %llx failed %d", r->id, id, rv);
        goto err;
    }
    ...
    if (id == 0) {
        ...
        if (id == 0) {
            ...
            evtErrf("raft(%llx) select transferee failed %d", r->id, rv);
            goto err;
        }
    }
    ...
    if (progressIsUpToDate(r, i)) {
        ...
        if (rv != 0) {
            r->transfer = NULL;
            evtErrf("raft(%llx) transfer to %llx failed %d", r->id, id, rv);
            goto err;
        }
    }
